...
```mermaid
graph TB
    subgraph Phase1["1. Mark 단계: 루트부터 도달 가능한 객체 표시"]
    ...
    subgraph Phase2["2. Sweep 단계: 표시되지 않은 객체 제거"]
...
// - result(localUser): 반환되어 루트에서 접근 가능 → 유지
...
그리고 마침내 처음의 질문으로 돌아왔을 때, 결국 중요한 것은 성능적인 부분이 아니라 코드의 의도라는 것을 느끼게되었습니다. 변하지 않는 상수라면 컴포넌트 외부에 선언하여 "이 값은 모든 인스턴스가 공유합니다"라는 의도를 명확히 하고, props나 state에 의존한다면 컴포넌트 내부에 두되 필요하면 `useMemo`로 최적화하면 되는 것이었습니다.
